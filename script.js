...
function categorizeAnime(anime) {
    ...
    // Genre string'ini al ve normalize et (Türkçe karakter sorununu çözmek için)
    // Örn: "İsekai" -> "isekai", "Korku" -> "korku", "Aksiyon" -> "aksiyon"
    const rawGenres = anime.genres || '';
    const genres = normalizeString(rawGenres);

    // Debug için
    // console.log(`Anime: ${anime.title}, Raw: ${rawGenres}, Normalized: ${genres}`);
    ...
    if (genres.includes('romance') || genres.includes('romantik') || genres.includes('ask')) categories.push('romance');
    if (genres.includes('horror') || genres.includes('korku') || genres.includes('gerilim')) categories.push('horror');
...
function normalizeString(str) {
    ...
    // Özel olarak büyük I ve İ karmaşasını çöz
    // Standart toLowerCase 'I'yı 'i' yapar ama Türkçe locale'de 'ı' yapabilir.
    // Biz manuel olarak replace yapalım.

    // Önce orijinal string üzerinden manuel dönüşüm yapmak daha güvenli
    normalized = str.replace(/[çğıiöşüÇĞIİÖŞÜ]/g, function (match) {
...
class AnimeCarousel {
    ...
    initializeButtons() {
        ...
        // Mouse Sürükleme Desteği
        this.initDragSupport(grid);
    }

    initDragSupport(grid) {
        let isDown = false;
        let startX;
        let scrollLeft;
        let diff = 0;
        ...
        grid.addEventListener('mousedown', (e) => {
            ...
            // Mevcut transform değerinden offset'i bul
            const transformMatrix = window.getComputedStyle(grid).transform;
            const currentTranslateX = transformMatrix !== 'none' ? parseInt(transformMatrix.split(',')[4]) : 0;
        ...
        grid.addEventListener('mouseup', () => {
            ...
            // Eğer sürükleme çok küçükse (tıklama gibi), snap yapma
            if (Math.abs(diff) > 5) {
                this.snapToNearest();
            }
        });
        ...
        grid.addEventListener('mousemove', (e) => {
            ...
            diff = walk;
            grid.style.transform = `translateX(${scrollLeft + walk}px)`;
        });
    }
    ...
    snapToNearest() {
        ...
        // En yakın index'i hesapla (Negatif olduğu için - ile çarpıp ve yuvarlıyoruz)
        let newIndex = Math.round(-currentTranslateX / itemWidth);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (searchInput) {
        ...
        searchInput.addEventListener('input', (e) => {
            ...
            searchTimeout = setTimeout(async () => {
                try {
                    const results = await AnimeAPI.searchAnimes(query);
        ...
        // Focus geri geldiğinde eğer input doluysa tekrar aç
    ...
    function renderSearchResults(results, container) {
        ...
        if (!results || results.length === 0) {
            container.innerHTML = '<div class="no-notifications" style="padding:15px;">Sonuç bulunamadı</div>';
        ...
        results.forEach(anime => {
            ...
            item.innerHTML = `
                <img src="${anime.poster_url || 'https://via.placeholder.com/40x60'}" alt="${anime.title}">
                <div class="search-item-info">
                    <h4>${anime.title}</h4>
                    <p>${anime.genres || 'Tür Yok'}</p>
                </div>
            `;
